...
| 方式          | 操作                                                                 |
| ----------- | ------------------------------------------------------------------ |
| **命令行（推荐）** | `cursor --install-extension "路径/kagent-0.1.7.vsix"`                |
...
- **右侧图表**：选中文件的 K 线；点列表项在编辑器打开该文件（已删除的不会弹错）；拖动/缩放后会保留视图，停在最新 K 线附近时保存会自动跟随  
- **右上角**：切换 **A 股 / 美股**、**亮 / 暗**（或设置 `kagent.colorScheme`、`kagent.colorTone`）
- **ST 退市**：在资源管理器中**手动删除**已跟踪文件后，列表项变灰并标 **ST退市**；稳定展示约 30 秒后自动从列表移除（历史 K 线仍保留在 `events.ndjson`）

Agent 每改一个新文件，列表里会多一支「新上市」的股票。若开启 `kagent.capture.onSave`，手动保存和手动新建工作区文件也会记入行情。
...
| ST 退市  | **手动删除**已跟踪的源文件；灰色显示，约 30 秒后下市 |
...
| `.kagent/symbols.json`  | 已「上市」文件列表（含 `delisted` / `delisted_at` 退市状态） |
